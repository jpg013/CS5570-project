...
def serializable_or_not(hist):
    '''
    pass a history
    returns a tuple [0] is bool for serializable or not
                    [1] is list of conflicting transactions
    '''
    ...
    G = nx.DiGraph(l)
    try:
        t = list(nx.find_cycle(G)) #raises an exception if a cycle is NOT found
...
def test_serializable():
    ...
    serial_res = serializable_or_not(serializable_hist)[0]
   
    not_serial_res = serializable_or_not(not_serializable_hist)[0]
