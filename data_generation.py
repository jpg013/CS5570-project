...
def get_transaction_data_cardinality():
...
def make_transaction_data_sample():
    # returns list of data_items
    data_sample = []
    data_cardinality = get_transaction_data_cardinality()

    while len(data_sample) < data_cardinality:
        data = generate_data_item()
...
def generate_transactions():
    ...
    transaction_cardinality = get_transaction_cardinality()
    transactions = []

    for idx in range(transaction_cardinality):
        tx = Transaction(idx+1)
        
        for x in generate_tx_data_operations(tx, make_transaction_data_sample()):
...
def generate_tx_data_operations(transaction, data_items):
    if len(data_items) < 1:
    ...
    for item in data_items:
        ...
        data_operations = data_operations + list(map(lambda type: DataOperation(type, transaction, item), generate_read_writes_types()))
    
    # Randomly shuffle all the data operations.
    random.shuffle(data_operations)

    # Add a commit / abort data operation at the end
    data_operations.append(DataOperation(generate_commit_abort_type(), transaction, None))
