...
def generate_data_operation(tx=None) {
    if tx is None:
        tx = Transaction(random.randint(
            Transaction.get('transaction_id_range').min,
            Transaction.get('transaction_id_range').max,
        ))
...
class DataGenerator():
    """DataGenerator is a helper class with several utility methods 
    for generating random data operations, transactions, and histories."""
